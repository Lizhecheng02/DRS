...
def llm_eval_answerable(context, question, eval_model):
    ...
    check = get_response_openai(model=eval_model, prompt=prompt, temperature=0.0, top_p=0.95, max_tokens=512)
    check = re.search(r"<answer>(.*?)</answer>", check, re.DOTALL).group(1).strip()
    return check


def get_response_openai(model, prompt, temperature=0.0, top_p=0.95, max_tokens=512):
...
def get_response_openai_reproduce(model, messages, temperature=0.0, top_p=0.95, max_tokens=512):
    completion = client_openai.chat.completions.create(
        model=model,
        messages=messages,
        temperature=temperature,
